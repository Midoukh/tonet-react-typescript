...
import { FilteringArrOfObjs } from "src/utils/helpers/arrays";
...
const ImageCard: FC<props> = ({ src, label, skeletonColor, endpoint }) => {
  ...
  const handleFetchCategoryContent = async () => {
    ...
    try {
      ...
      // dispatch(getSingleImageContent(response.data.media));
      const filteredArrayOfObj = FilteringArrOfObjs(response.data.media, [
        "url",
        "src",
      ]);
      console.log("[ImageCard] ===>> ", response.data.media);
      console.log("[ImageCard] after filtering===>> ", filteredArrayOfObj);
    } catch (error) {
      console.log(error);
      setIsFetching(false);
    }
