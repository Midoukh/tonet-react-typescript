...
const Calculator: FC = (props) => {
  ...
  const handleInput = (e: any) => {
    ...
    if (operation === "X") {
      ...
      handleRemoveEqualFromEx();

      handleShowResult();
    } else if (operation === "C") cleanView();

    if (!adjacentOps) {
      if (isPressedKeyOperator) {
        updatedInput = updatedInput.replace("=", "");
        updatedInput = updatedInput.replace("C", "");
        if (operation !== "C") updatedInput = ` ${updatedInput} `;
  ...
  const handleRemoveEqualFromEx = () => {
    const updatedInput = newCalculator.removeEqualFromExpression(input);

    console.log("calling this stupid bitch func with new inpt", updatedInput);
    setInput(updatedInput);
  };
  const cleanView = () => {
    setInput("0");
